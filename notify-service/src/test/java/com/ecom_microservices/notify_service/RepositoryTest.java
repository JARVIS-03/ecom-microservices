...
public class RepositoryTest {
    ...
    @Test
    void testFindNotificationByStatusOrderByPriority() {
        Pageable pageable = PageRequest.of(0, 10);

        Notification highPriority = new Notification();
        highPriority.setRecipient("high@example.com");
        highPriority.setStatus(NotificationStatus.PENDING);
        highPriority.setPriority(PriorityLevel.HIGH);
        highPriority.setCreatedTimestamp(LocalDateTime.now().minusHours(1));

        Notification lowPriority = new Notification();
        lowPriority.setRecipient("low@example.com");
        lowPriority.setStatus(NotificationStatus.PENDING);
        lowPriority.setPriority(PriorityLevel.LOW);
        lowPriority.setCreatedTimestamp(LocalDateTime.now());

        List<Notification> mockResult = List.of(highPriority, lowPriority);

        when(notificationRepository.findNotificationByStatusOrderByPriority(NotificationStatus.PENDING, pageable))
                .thenReturn(mockResult);

        List<Notification> results = notificationRepository.findNotificationByStatusOrderByPriority(NotificationStatus.PENDING, pageable);

        assertNotNull(results);
        assertEquals(2, results.size());
        assertEquals(PriorityLevel.HIGH, results.get(0).getPriority());
        assertEquals(PriorityLevel.LOW, results.get(1).getPriority());

        verify(notificationRepository, times(1)).findNotificationByStatusOrderByPriority(NotificationStatus.PENDING, pageable);
    }
}